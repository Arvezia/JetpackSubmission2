...
class RemoteDataSource {
    ...
    suspend fun getMovieShows(callback: LoadMoviesCallback){
        ...
        ApiConfig.instance.getMovieList().await().result?.let { listMovie -> callback.moviesReceived(listMovie)
            EspressoIdlingResources.decrement()
        }
    }
   suspend fun getTVShows(callback: LoadTVCallback){
        EspressoIdlingResources.increment()
        ApiConfig.instance.getTvShowList().await().result?.let { listTVShows -> callback.tvReceived(listTVShows)
            EspressoIdlingResources.decrement()
        }
    }
